...
const SubstituteClass = {
  ...
  async findById(id) {
    const [rows] = await pool.execute('SELECT * FROM substitute_classes WHERE id = ?', [id]);
    return rows[0];
  },
  ...
  async create(data) {
    const [result] = await pool.execute(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `, [
      data.student_id, data.original_teacher_id, data.substitute_teacher_id,
      data.class_date, data.class_time, data.duration_minutes, data.reason,
  ...
  async update(id, data) {
    ...
    `, [
      data.student_id, data.original_teacher_id, data.substitute_teacher_id,
      data.class_date, data.class_time, data.duration_minutes,
      data.reason, data.status, data.notes, id
    ]);
    return true;
  },

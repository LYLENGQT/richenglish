...
const Attendance = {
  ...
  async create(data) {
    const [result] = await pool.execute(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `, [
      data.class_id, data.student_id, data.teacher_id, data.date, data.status, data.minutes_attended, data.notes
    ]);
  ...
  async update(id, data) {
    await pool.execute(`
      ...
      WHERE id=?
    `, [
      data.class_id, data.student_id, data.teacher_id, data.date,
      data.status, data.minutes_attended, data.notes, id
    ]);
    return true;
  },
